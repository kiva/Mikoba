...
namespace mikoba.Services
{
    public class NavigationService : INavigationService
    {
        ...
        public async Task AddTabChildToMainView<TViewModel>(TViewModel viewModel, object parameter, int atIndex = -1) where TViewModel : IBaseViewModel
        {
            var mainPage = CurrentApplication.MainPage as TabbedPage;
        ...
        public IList<IBaseViewModel> GetMainViewTabChildren()
        {
            var mainPage = CurrentApplication.MainPage as TabbedPage;
            ...
            return mainPage.Children.Select(_ => (IBaseViewModel)_.BindingContext).ToList();
        }
        ...
        public Task NavigateToAsync<TViewModel>(TViewModel viewModel, object parameter = null, NavigationType type = NavigationType.Normal) where TViewModel : IBaseViewModel => InternalNavigateToAsync(typeof(TViewModel), type, viewModel, null);
        
        public Task NavigateToAsync<TViewModel>(object parameter, NavigationType type = NavigationType.Normal) where TViewModel : IBaseViewModel => InternalNavigateToAsync(typeof(TViewModel), type, null, parameter);
        ...
        public Task NavigateToPopupAsync<TViewModel>(bool animate, TViewModel viewModel) where TViewModel : IBaseViewModel => NavigateToPopupAsync(null, animate, viewModel);

        public async Task NavigateToPopupAsync<TViewModel>(object parameter, bool animate, TViewModel vieModel) where TViewModel : IBaseViewModel
        {
        ...
        protected Page CreateAndBindPage(Type viewModelType, object viewModelObj, object parameter, bool isPopup)
        {
            ...
            if (viewModelObj != null)
                viewModel = viewModelObj as IBaseViewModel;
            else
                viewModel = _scope.Resolve(viewModelType) as IBaseViewModel;
            page.BindingContext = viewModel;
            
            return page;
        }

        public void AddPageViewModelBinding<TViewModel, TPage>() => Mappings.Add(new Tuple<Type, Type, bool>(typeof(TViewModel), typeof(TPage), false));

        public void AddPopupViewModelBinding<TViewModel, TPage>() => Mappings.Add(new Tuple<Type, Type, bool>(typeof(TViewModel), typeof(TPage), true));
    }
}
